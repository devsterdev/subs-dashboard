...
const Home = ({name, uname, subs, url}) => {
  return (
    ...
    <div className="flex gap-5 items-center flex-col sm:flex-row text-center sm:text-left">
      ...
      />
      <div>
        <h3 className="text-3xl sm:text-4xl font-black uppercase leading-none tracking-tight">{name}</h3>
        <p className="text-lg font-bold uppercase text-zinc-600 mt-1.5">{uname}</p>
      </div>
    </div>
    <div className="flex flex-col items-center sm:items-end gap-3 min-w-[120px] w-full sm:w-auto">
      <span className="text-xl font-black uppercase border-b-[3px] border-black pb-1.5">{subs}</span>
